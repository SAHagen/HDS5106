data {
  ...
  int<lower=0, upper=1> prior_only;       // ADD THIS
}
...
model {
  ...
  if (prior_only == 0) {                  // ADD THIS WRAPPER
...
generated quantities {
  ...
  vector[N_obs] y_rep;
  for (j in 1:N_obs) {
    y_rep[j] = normal_rng(theta[obs_idx[j]], psi[j]);
  }
  
  vector[N_obs] log_lik;
  for (j in 1:N_obs) {
    log_lik[j] = normal_lpdf(y[j] | theta[obs_idx[j]], psi[j]);
  }
}
