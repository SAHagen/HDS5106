...
- Elevation
  - Hydrologic Derivatives for Modeling and Analysis elevation raster
- Population + Density, literacy, and poverty
  - Ghana Census 2021
- Precipitation
  - World Food Programme Climate Hazards Group InfraRed Precipitation satellite imagery
  - The 3 month average was taken for each time point to account for the duration of the DHS and census data collection 
- Travel Time 
...
Elevation rasters did not fall along clean country boundaries, so multiple rasters were combined and trimmed for each of the countries involved. Raster data for elevation and travel time were pulled and averaged over district level for each country. Rainfall data was sorted for each district and the date corresponding with the end of the census or DHS survey was chosen. Missing districts were assigned NA values. Poverty data in Ghana only exists in the 2021 census, thus the poverty value takes on NA values in the 2010 census.
...
direct_est_ghana22 <- direct_est_all %>%
  filter(iso == "GHA", year == 2022)

write.csv(direct_est_ghana22, file = "direct_est_ghana22.csv", row.names = FALSE)
...
build_panel <- function(country_iso, district_lookup, direct_est_all) {
  ...
  est_clean <- direct_est_all %>%
    filter(country == country_iso) %>%
    select(district_id, year, y_i, psi_i) # we only need the logit prevalence and its standard error for the model, along with the district_id and year for merging onto the panel. We also filter to the specific country we are building the panel for.
...
```{r covariates pre, echo=TRUE}
 ghana_cov_2022 <- read_csv("ghana_district_covariates_2022.csv") %>%
  select(
    district_code,
    elev_mean,
    precip_mean,
  ...
  mutate(
    # Log-transform population density BEFORE standardising
    log_pop_density = log(pop_density)
  )

# Standardise all covariates, using log version for pop density
ghana_cov_2022_std <- ghana_cov_2022 %>%
  mutate(
    elevation_std       = as.numeric(scale(elev_mean)),
    rainfall_std        = as.numeric(scale(precip_mean)),
    travel_time_std     = as.numeric(scale(travel_time_mean)),
    poverty_std         = as.numeric(scale(poverty_H)),
    literacy_rate_std   = as.numeric(scale(literacy_rate)),
    pop_density_std     = as.numeric(scale(log_pop_density))   # standardise the LOG
  ) %>%
  select(district_code, ends_with("_std"))

write.csv(ghana_cov_2022_std, file = "ghana_cov_2022_std.csv", row.names = FALSE)
...
```{r covariates_join_2022, echo=TRUE}
ghana_cov_2022_indexed <- district_lookups[["GHA"]] %>%
  select(district_id, district_code, district_name) %>%
  inner_join(ghana_cov_2022_std, by = "district_code")


#joinging for Ghana 2022 panel
panels[["GHA"]] <- panels[["GHA"]] %>%
  left_join(ghana_cov_2022_indexed %>% 
              select(district_id, ends_with("_std")), 
            by = "district_id")

ghanA_FILE <- panels[["GHA"]] %>%
  filter(year == 2022) %>%
  select(district_id, district_name, elevation_std, rainfall_std, travel_time_std,
         poverty_std, literacy_rate_std, pop_density_std, observed, y_i, psi_i)

#
write.csv(ghanA_FILE, file = "ghanA_FILE.csv", row.names = FALSE)
...
build_stan_data_st <- function(iso, panels, spatial, 
                            covar_cols = NULL) {
...
build_stan_data_cs <- function(iso, panels, spatial, year, covar_cols = NULL, prior_only = 1L) {
  
  # Filter panel to the specific year
...
stan_data[["GHA_CS_2022"]] <- build_stan_data_cs(
  ...
  spatial = spatial,
  year = 2022,
  covar_cols = c("elevation_std", "rainfall_std", "travel_time_std",
...
mod_cs <- cmdstan_model("prior_predictive_checks.stan")
...
fit_gha_cs$cmdstan_diagnose()
```

```{r posterior predictive checks, echo=TRUE}
# =============================================================================
# 6. POSTERIOR PREDICTIVE CHECKS (after fitting the full model)
# =============================================================================

y_rep_post <- fit_gha_cs$draws("y_rep", format = "matrix")
prev_post  <- fit_gha_cs$draws("prev", format = "matrix")
# CHECK 1: Density overlay — does y_rep match y_obs?
ppc_dens_overlay(y = stan_data$y, yrep = y_rep_post[1:100, ]) +
   labs(title = "Posterior Predictive Check: y_rep vs Observed y")
#
# # CHECK 2: Mean
ppc_stat(y = stan_data$y, yrep = y_rep_post, stat = "mean") +
   labs(title = "PPC: Mean of y")
#
# # CHECK 3: Standard deviation
ppc_stat(y = stan_data$y, yrep = y_rep_post, stat = "sd") +
  labs(title = "PPC: SD of y")
#
# # CHECK 4: Intervals per district
ppc_intervals(y = stan_data$y, yrep = y_rep_post,
               x = 1:N, prob = 0.5, prob_outer = 0.95) +
   labs(title = "PPC: Posterior Predictive Intervals by District",
        x = "District", y = "Direct estimate (logit scale)")
#
# # CHECK 5: Posterior predictive p-values
ppp <- colMeans(y_rep_post >= matrix(stan_data$y, nrow(y_rep_post), N, byrow = TRUE))
hist(ppp, breaks = 30, main = "Posterior Predictive p-values",
      xlab = "p-value", col = "steelblue")
# # Should be roughly uniform. Pile-up near 0 or 1 = misfit.
#
# # CHECK 6: Residual spatial correlation
resid_post <- colMeans(y_rep_post) - stan_data$y
moran.test(resid_post, listw = nb2listw(nb))

# # Significant Moran's I → model missing spatial signal
